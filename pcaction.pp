...
begin
	NPC := LocatePilot( NPC );
	if ( NPC <> Nil ) and GearOperational( NPC ) and AreEnemies( GB , NPC , PC ) and NotAnAnimal( NPC ) and IsFoundAlongTrack( GB^.Meks , NPC ) then begin
...
begin
	...
	if ( MM <> 0 ) and ( MM <> CMM ) then begin
		...
		DialogMsg( ReplaceHash( MsgString( 'ShiftGears' ) , MsgString( 'MOVEMODENAME_' + BStr( MM ) ) ) );
	end else begin
		DialogMsg( MsgString( 'SHIFTGEARS_FAIL' ) );
	end;
end;
