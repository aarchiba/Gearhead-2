...
Function MightSurrender( GB: GameBoardPtr; NPC: GearPtr ): Boolean;
	...
	{ current situation, or FALSE otherwise. }
begin
	MightSurrender := ( CurrentStamina(NPC) = 0 ) and AreEnemies(GB,NAttValue(NPC^.NA,NAG_Location,NAS_Team),NAV_DefPlayerTeam) and (NAttValue(NPC^.NA,NAG_EpisodeData,NAS_SurrenderStatus) = 0) and NotAnAnimal( NPC );
end;
...
begin
	...
	if ( Target^.G = GG_Character ) and MightSurrender( GB , Target ) then begin
		...
		AtRoll :=  SkillRoll( Attacker , AtSkill , DefRoll , -NAttValue( Target^.NA , NAG_EpisodeData , NAS_TauntResistance ) , False );
		...
		AtRoll :=  SkillRoll( Attacker , NAS_Taunt , DefRoll , 0 , False );
