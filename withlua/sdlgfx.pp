...
const
	...
	ZONE_Dialog: TSDL_Rect = ( x: 30; y: ScreenHeight - Dialog_Height - 10; w: ScreenWidth - 60 ; h: Dialog_Height );
...
begin
	...
	SDL_SetColorKey( MyImage2 , SDL_SRCCOLORKEY or SDL_RLEACCEL , SDL_MapRGB( MyImage2^.Format , 255 , 0, 255 ) );
...
begin
	...
	if name <> '' then begin
		...
		if it^.Img <> Nil then begin
			...
			SDL_SetColorKey( it^.Img , SDL_SRCCOLORKEY or SDL_RLEACCEL , SDL_MapRGB( it^.Img^.Format , 255 , 0, 255 ) );
...
initialization
	...
	ClrScreen;
	SDL_SetColorKey( Game_Screen , SDL_SRCCOLORKEY or SDL_RLEACCEL , SDL_MapRGB( Game_Screen^.Format , 255 , 0 , 255 ) );
