...
uses ability,aibrain,arenacfe,arenascript,backpack,gamebook,ghmodule,ghholder,
     ghchars,ghprop,ghweapon,menugear,movement,pcaction,
     randmaps,rpgdice,skilluse,texutil,uiconfig,wmonster,
     action,narration,gearparser,customization,plotsearch,plotbuild,
{$IFDEF ASCII}
	vidmap,vidgfx;
{$ELSE}
	sdlmap,sdlgfx;
...
begin
	...
	if RootScene <> Nil then begin
		PublicScene := ElementSearch( RootScene , Nil , GG_Scene , ':PUBLI (BUILD|MEETI)' , Nil );
