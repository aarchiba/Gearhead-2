...
begin
	...
	if NotAnAnimal( NPC ) and ( ( Msg_Label = 'CHAT_EJECT' ) or ( NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) <> 0 ) ) then begin
...
begin
	...
	if NAttValue( Mek^.NA , NAG_Narrative , NAS_NID ) <> 0 then begin
		SetTrigger( GB , TRIGGER_NPCSurrendered + BStr( NAttValue( Mek^.NA , NAG_Narrative , NAS_NID ) ) );
	end;
