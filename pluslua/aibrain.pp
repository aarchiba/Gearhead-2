...
uses ability,action,arenacfe,effects,movement,gamebook,specialsys,ghswag,
     ghchars,ghmodule,ghweapon,gearparser,ghprop,interact,rpgdice,skilluse,
     texutil,uiconfig,arenascript,menugear,ghsupport,backpack,narration,
{$IFDEF ASCII}
	vidgfx,vidmap;
{$ELSE}
	sdlmap,sdlgfx;
...
var
	...
	begin
		if ( NAttValue( M^.NA , NAG_Narrative , NAS_NID ) = 0 ) or ( NAttValue( M^.NA , NAG_Location , NAS_Team ) = Team ) or ( NAttValue( M^.NA , NAG_Location , NAS_Team ) = NAV_DefPlayerTeam ) then begin
...
var
	...
	begin
		...
		if ( NPC <> Nil ) and ( NAttValue( NPC^.NA , NAG_Personal , NAS_LMOrders_OptMax ) <> 0 ) then begin
			N1 := NAttValue( NPC^.NA , NAG_Personal , NAS_LMOrders_OptMax );
			N2 := NAttValue( NPC^.NA , NAG_Personal , NAS_LMOrders_OptMin );
		end else begin
