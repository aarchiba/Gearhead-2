...
uses dos,rpgdice,texutil,gears,ghmecha,ghmodule,ghchars,ghweapon,ghsupport,gamebook,locale,interact,ability,uiconfig,ghholder,movement;
...
begin
	if NAttValue( NPC^.NA , NAG_Personal , NAS_SpecialistSkill ) <> 0 then begin
		RecordError( 'WARNING: ' + GearName( NPC ) + '/' + SAttValue( NPC^.SA , 'JOB' ) + '/' + BStr( NAttValue( NPC^.NA , NAG_Personal , NAS_FactionID ) ) + ' had spec skill ' + BStr( NAttValue( NPC^.NA , NAG_Personal , NAS_SpecialistSkill ) ) + ' set.' );
	end;
	...
	else RecordError( 'ERROR: No theme found for ' + SkList );
end;
...
Function ReadGear( var F: Text; RandomizeNPCs: Boolean; const ZZZFName: String ): GearPtr;
	...
	{ in gearutil.pp. }
	{ ZZZFName is the name of the file, to be used in debugging. }
const
	NDum: GearPtr = Nil;
var
...
begin
	...
	if CM_FX <> '' then begin
		...
		RecordError( 'ERROR: Command ' + UpCase( CM_CMD ) + ' not found in ' + ZZZFName );
		RecordError( '--"' + OriginalLine + '"' );
	end;
end;
...
Procedure CMD_Lua;
	{ We have encountered a Lua block. Deal with it. }
var
	CL_Line: String;
	CL_KeepGoing: Boolean;
begin
	if C <> Nil then begin
		{ Keep going until we reach a line that says "EndLua" }
		CL_KeepGoing := True;
		while CL_KeepGoing and not EoF(F) do begin
			{Read the line from disk, and delete leading whitespace.}
			readln(F,CL_Line);
			DeleteWhiteSpace(CL_Line);
			if CL_Line <> '' then begin
...
initialization
	...
	Factions_List := AggregatePattern( 'FACTIONS_*.txt' , Setting_Directory );
	Mecha_Theme_List := AggregatePattern( 'THEME_*.txt' , Setting_Directory );
