...
uses 	gamebook,ghchars,texutil,arenacfe,ability,uiconfig,menugear,
...
begin
	...
	if BP > 0 then begin
		...
		if Form^.Stat[ STAT_Charm ] > 1 then begin
			SetNAtt( Form^.NA , NAG_Narrative , NAS_NID , NewNID( FindRoot( GB^.Scene ) ) );
...
begin
	...
	if Robot = Nil then begin
		...
		if NAttValue( Robot^.NA , NAG_Narrative , NAS_NID ) <> 0 then begin
...
initialization
	...
	Robotic_Forms := AggregatePattern( 'ROBOTS_*.txt' , Setting_Directory );
