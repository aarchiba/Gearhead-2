...
Const
	...
	{ NAG_ReactionScore records how well two characters like each other or }
	{ hate each other. The "S" identifier is the NID of the character }
	{ to which this reaction score applies. }

	{ NAG_FactionScore records the relationship between two factions. }
...
uses narration,texutil,rpgdice,ghchars,gamebook,ability,uiconfig,ghprop,action;
...
begin
	...
	Persona := NAttValue( NPC^.NA , NAG_Narrative , NAS_NID );
	PC := LocatePilot( PC );
	...
	if NAttValue( NPC^.NA , NAG_Relationship , NAttValue( PC^.NA , NAG_Narrative , NAS_NID ) ) = NAV_ArchEnemy then begin
		it := it - ArchEnemyReactionPenalty;
		if it > 0 then it := 0;
	end else if NAttValue( NPC^.NA , NAG_Relationship , NAttValue( PC^.NA , NAG_Narrative , NAS_NID ) ) > 0 then begin
...
begin
	NPC := LocatePilot( NPC );
	IsRegularLancemate := ( NPC <> Nil ) and ( NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) <> 0 ) and ( NAttValue( NPC^.NA , NAG_CharDescription , NAS_CharType ) <> NAV_TempLancemate );
end;
...
begin
	...
	N := 0;
	while M <> Nil do begin
		if ( NAttValue( M^.NA , NAG_Location , NAS_Team ) = NAV_LancemateTeam ) and ( M^.G = GG_Character ) and ( NAttValue( M^.NA , NAG_Narrative , NAS_NID ) = 0 ) then begin
			Inc( N );
		end;
		M := M^.Next;
	end;
...
Function FindLocalNPCByKeyWord( GB: GameBoardPtr; KW: String ): GearPtr;
	...
	begin
		...
		Persona := SeekPersona( GB , NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) );
...
begin
	if ( PC <> Nil ) and ( NPC <> Nil ) and ( GB <> Nil ) then begin
		...
		AddNAtt( PC^.NA , NAG_ReactionScore , NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) , DReact );
