...
uses ability,action,aibrain,arenacfe,arenascript,backpack,
     effects,gamebook,targetui,ghchars,gearparser,robotics,
     ghprop,ghswag,ghweapon,interact,menugear,movement,
     rpgdice,skilluse,texutil,uiconfig,
	...
	if ( FXP > 0 ) and ( NAttValue( NPC^.NA , NAG_Experience , NAS_TotalXP ) > FXP ) then begin
		...
		if N > 0 then begin
			...
			if ( M2 <> Nil ) and ( NPC^.Parent = Nil ) then begin
				...
				SetNAtt( M^.NA , NAG_Narrative , NAS_NID , NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) );
...
begin
	...
	if ( NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) <> 0 ) then AddRPGMenuItem( RPM , MsgString( 'FHQ_SelectMecha' ) , 4 );
	if ( NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) = 0 ) or ( UpCase( SAttValue( NPC^.SA , 'JOB' ) ) = 'ROBOT' ) then AddRPGMenuItem( RPM , MsgString( 'FHQ_Rename' ) , 5 );
	if ( NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) = 0 ) and ( NAttValue( NPC^.NA , NAG_Location , NAS_Team ) <> NAV_LancemateTeam ) then AddRPGMenuItem( RPM , MsgString( 'FHQ_Rejoin' ) , 6 );
	...
	if ( NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) = 0 ) and ( UpCase( SAttValue( NPC^.SA , 'JOB' ) ) = 'ROBOT' ) then AddRPGMenuItem( RPM , MsgString( 'FHQ_Disassemble' ) , 7 );
	...
	repeat
		...
		case N of
			...
			2: 	if NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) <> 0 then begin
					PCACTIONRD_GB := GB;
...
begin
	...
	repeat
		...
		while M <> Nil do begin
			...
			end else if ( NAttValue( M^.NA , NAG_CharDescription , NAS_CharType ) = NAV_CTLancemate ) and ( NAttValue( M^.NA , NAG_Narrative , NAS_NID ) = 0 ) Then begin
				AddRPGMenuItem( RPM , TeamMateName( M ) , N );
	...
	while MT <> Nil do begin
		...
		if ( MT^.G = GG_MetaTerrain ) and ( MT^.Stat[ STAT_MetaVisibility ] > 0 ) and ( Range( MT , P.X , P.Y ) <= 1 ) then begin
			...
			if SkillRoll( GB , Mek , NAS_Awareness , STAT_Perception , MT^.Stat[ STAT_MetaVisibility ] , 0 , False , True ) > MT^.Stat[ STAT_MetaVisibility ] then begin
				...
				TriggerGearScript( GB , MT , 'reveal' );
				VisionCheck( GB , Mek );
			end;
		end;
...
Procedure UsePropFrontEnd( GB: GameBoardPtr; PC , Prop: GearPtr; const T: String );
...
Procedure DominateAnimal( GB: GameBoardPtr; PC: GearPtr );
	...
	begin
		if GearActive( M ) and AreEnemies( GB , M , PC ) and ( NAttValue( M^.NA , NAG_Narrative , NAS_NID ) = 0 ) then begin
			IsGoodTarget := True;
		end else if GearActive( M ) and ( NAttValue( M^.NA , NAG_Narrative , NAS_NID ) = 0 ) and ( NAttValue( M^.NA , NAG_CharDescription , NAS_CharType ) = NAV_CTLancemate ) and ( NAttValue( M^.NA , NAG_Location , NAS_Team ) <> NAV_LancemateTeam ) then begin
			IsGoodTarget := True;
		end else begin
...
Procedure PickPockets( GB: GameBoardPtr; PC: GearPtr );
	...
	var
		Team: Integer;
	begin
		if GearActive( M ) and ( M^.G = GG_Character ) and ( NAttValue( M^.NA , NAG_Narrative , NAS_NID ) <> 0 ) then begin
	...
	if Target = Nil then begin
		...
		if SkRoll > SkTarget then begin
			...
			if SkRoll < ( SkTarget - 10 ) then begin
				...
				AddNAtt( PC^.NA , NAG_ReactionScore , NAttValue( Target^.NA , NAG_Narrative , NAS_NID ) , -20 );
			end;
		end;
		...
		{ up poorer...? }
		if SkRoll < ( SkTarget + 10 ) then AddNAtt( PC^.NA , NAG_ReactionScore , NAttValue( Target^.NA , NAG_Narrative , NAS_NID ) , -( 5 + Random(10) ) );
...
begin
	...
	Weapon := LocateGearByIndex( Mek , NAttValue( Mek^.NA , NAG_Location , NAS_SmartWeapon ) );
	if ( T = -1 ) and OnTheMap( GB, TX , TY ) then begin
		...
		if N > 0 then begin
			Enemy := LocateGearByIndex( Enemy , N );
			AtOp := 0;
		end else begin
	...
	if SelectTarget( GB , Mek , Weapon , CallShot , AtOp ) then begin
		...
		if ( LOOKER_Gear = Nil ) and RangeArcCheck( GB , Mek , Weapon , LOOKER_X , LOOKER_Y , TerrMan[ TileTerrain( GB , LOOKER_X , LOOKER_Y ) ].Altitude ) then begin
			...
			if CallShot and ( LOOKER_Gear^.SubCom <> Nil ) then begin
				...
				if N <> -1 then begin
					LOOKER_Gear := LocateGearByIndex( LOOKER_Gear , N );
				end;
				DisposeRPGMenu( WPM );
				AtOp := 0;
			end;
...
begin
	...
	while MI <> Nil do begin
		...
		Weapon := LocateGearByIndex( Mek , MI^.Value );
	...
	if N > -1 then begin
		{ A weapon has been selected. Now, select a target. }
		Weapon := LocateGearByIndex( Mek , N );
