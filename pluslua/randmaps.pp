...
uses gamebook,ghprop,rpgdice,texutil,gearparser,narration,uiconfig,ghchars,sysutils,ability,
{$IFDEF ASCII}
	vidgfx;
{$ELSE}
	sdlgfx;
...
var
	...
	it: GameBoardPtr;
begin
	if FileExists( Setting_Directory + FName ) then begin
		Assign( F , Setting_Directory + FName );
		Reset( F );
...
begin
	...
	{ the work here has already been done for us. Pick one of the cells }
...
begin
	...
	if DoorPrototype <> Nil then begin
		NewDoor := InstantGear( DoorPrototype );
	end else begin
		NewDoor := NewGear;
...
begin
	...
	if GB^.Scene^.G = GG_MetaScene then begin
		...
		Entry^.Stat[ STAT_Destination ] := NAttValue( GB^.Scene^.NA , NAG_Narrative , NAS_EntranceScene );
	end else begin
		Entry^.Stat[ STAT_Destination ] := NAttValue( GB^.Scene^.Parent^.NA , NAG_Narrative , NAS_NID );
...
begin
	...
	SPType := SAttValue( SPReq^.SA , 'REQUIRES' );
	AddGearXRContext( GB , FindRoot( GB^.Scene ) , GB^.Scene , SPType , '@' );
	...
	while P <> Nil do begin
		...
		AppendStringList( P^.Scripts , SPReq^.Scripts );
		ActivateGearScript( P );
...
Procedure InsertContentFragment( GB: GameBoardPtr; Adv,C,Zone: GearPtr );
	{ We're going to finalize the insertion of this particular fragment. Yay! }
	{ - Move all physical elements which do not have a PLACE defined. }
	{ - Convert MetaPersonas to real personas and stick them in the scene. }
	{ - If this content has scripts, convert to script object and store in scene. }
	{ - Copy over MiniMap info and Special attribute. }
	{ - Copy over the Door prototype. }
var
	E,P,P2,Team,Door: GearPtr;
	T: Integer;
	Loc: Point;
	MiniMap,Z_Special,TeamData: String;
begin
	{ Copy over the MiniMap+Special now. }
	if Zone <> Nil then begin
		MiniMap := SAttValue( C^.SA , 'MINIMAP' );
		if ( MiniMap <> '' ) then SetSAtt( Zone^.SA , 'MINIMAP <' + MINIMAP + '>' );
	...
	{ Finally, deal with the script object and leftover prefabs. }
	if ( C^.Scripts <> Nil ) then begin
		P := NewGear;
		P^.G := GG_RANDMAPSCRIPT;
		P^.SA := CloneSAtt( C^.SA );
		P^.Scripts := CloneSAtt( C^.Scripts );
		SetNAtt( P^.NA , NAG_Narrative , NAS_NID , NAttValue( C^.NA , NAG_Narrative , NAS_NID ) );
		SetNAtt( C^.NA , NAG_Narrative , NAS_NID , 0 );
		InsertSubCom( GB^.Scene , P );
		ActivateGearScript( P );
	end;
end;

Procedure InsertFinishedContent( GB: GameBoardPtr; Adv,C,Zone: GearPtr );
...
Function AddContent( CType: String; GB: GameboardPtr; Zone: GearPtr; P: String; var Cells: SAttPtr; SCheck,STerr: Integer ): Boolean;
...
begin
	...
	if C <> Nil then begin
		InsertFinishedContent( GB , FindRoot( GB^.Scene ) , C , Zone );
		{ Once it's been inserted, we can delete it. }
		RemoveGear( C^.Parent^.InvCom , C );
		AllOK := True;
			...
			if FCMode = MODE_Some then begin
				while Content_Num > 0 do begin
					if Random( 100 ) < Content_Chance then begin
						...
						AddContent( Content_Type , GB , Zone , Content_Parameter , Cells , SCheck , STerr );
					end;
					Dec( Content_Num );
				end;
