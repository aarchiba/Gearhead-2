...
uses rpgdice,gears,gamebook,movement,ability;
...
Function FindGearScene( Part: GearPtr; GB: GameBoardPtr ): Longint;
...
begin
	...
	if Part <> Nil then begin
		...
		if Part^.G = GG_Scene then begin
			...
			it := NAttValue( Part^.NA , NAG_Narrative , NAS_NID );
			...
			while P2 <> Nil do begin
				if P2 = Part then it := NAttValue( GB^.Scene^.NA , NAG_Narrative , NAS_NID );
				P2 := P2^.Next;
			end;
		end;
	end;
