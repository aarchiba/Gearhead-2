...
uses arenaplay,arenascript,interact,gamebook,narration,texutil,ghprop,rpgdice,ability,
     ghchars,ghweapon,movement,uiconfig,gearparser,plotbuild,randmaps,wmonster,
...
var
	...
	A: Char;
begin
	NPC := SeekGearByNID( Nil , Adv^.InvCom , ArenaNPC );
	repeat
		BasicArenaRedraw;
...
begin
	NPC := SeekGearByNID( Nil , Adv^.InvCom , ArenaNPC );
	ArenaNPCMessage := SAttValue( ANPC_MasterPersona^.SA , Msg_Key );
end;
...
begin
	...
	Camp := NewCampaign;
	Camp^.Source := LoadFile( 'arenastub.txt' , Setting_Directory );
