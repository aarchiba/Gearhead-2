...
uses ability,arenacfe,backpack,gamebook,ghchars,ghmodule,gearparser,
     ghswag,ghweapon,interact,menugear,rpgdice,skilluse,texutil,
     description,narration,uiconfig,ghprop,
     customization,
{$IFDEF ASCII}
...
begin
	...
	if ( SERV_Info <> Nil ) and ( Serv_Menu <> Nil ) then begin
		...
		if N > 0 then begin
			Part := LocateGearByIndex( SERV_Info , N );
			if Part <> Nil then begin
				BrowserInterfaceInfo( SERV_GB , Part , ZONE_ItemsInfo );
			end;
		end;
...
begin
	...
	if WasStolen then begin
		...
		if N >= 0 then begin
			...
			if ( PC <> Nil ) and ( NAttValue( PC^.NA , NAG_ReactionScore , NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) ) >= -20 ) then begin
				AddReputation( PC , 2 , -1 );
				if N > Random( 200 ) then AddReputation( PC , 6 , -1 );
				AddNAtt( PC^.NA , NAG_ReactionScore , NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) , -( Random( 6 ) + 1 ) );
			end;
...
begin
	...
	if ( Random( 200 ) + 10 ) < R then begin
		...
		AddNAtt( PC^.NA , NAG_ReactionScore , NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) , -Random( 10 ) );
		Cost := 0;
	end;
...
begin
	...
	if ( Random( 90 ) + 10 ) < R then begin
		...
		AddNAtt( PC^.NA , NAG_ReactionScore , NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) , -Random( 5 ) );
		Cost := 0;
	end;
...
begin
	...
	repeat
		...
		if N > -1 then begin
			Part := LocateGearByIndex( PCInv , N );
