...
uses ability,gamebook,ghweapon,menugear,texutil,uiconfig,ghsensor,
...
begin
	...
	BV := NAttValue( Weapon^.NA , NAG_Preferences , NAS_DefAtOp );
	if BV = 0 then begin
...
begin
	...
	if LOOKER_Weapon^.G = GG_Weapon then begin
		if LOOKER_Weapon^.S = GS_Missile then begin
			BV := BV + 1;
			if BV > 4 then BV := 1;
			SetNAtt( LOOKER_Weapon^.NA , NAG_Preferences , NAS_DefAtOp , BV );
