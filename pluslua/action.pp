...
uses 	ability,gamebook,ghchars,ghmodule,ghweapon,movement,rpgdice,texutil,
	math,uiconfig,ghintrinsic,ghsensor,ghprop,ghsupport,
{$IFDEF ASCII}
	vidgfx;
{$ELSE}
	sdlgfx;
	...
	begin
		...
		if Ok_At_Start and ( not GearOperational( Part ) ) then begin
			...
			while GoUp <> Nil do begin
				...
				{ If GoUp is destroyed and it had a NID, generate }
				{ a UTD* trigger. }
				if ( NAttValue( GoUp^.NA , NAG_Narrative , NAS_NID ) <> 0 ) and ( Not GearOperational( GoUp ) ) then begin
					SetTrigger( GB , TRIGGER_NPCEliminated + BStr( NAttValue( GoUp^.NA , NAG_Narrative , NAS_NID ) ) );
				end;
	...
	if Order = NAV_Stop then begin
		...
		case Order of
			NAV_NormSpeed,NAV_FullSpeed,NAV_Reverse:
				...
				Pilot := LocatePilot( Mek );
				if ( Pilot <> Nil ) and ( NAttValue( Pilot^.NA , NAG_Narrative , NAS_NID ) <> 0 ) then SetTrigger( GB , TRIGGER_NPCMovement + BStr( NAttValue( Pilot^.NA , NAG_Narrative , NAS_NID ) ) );
				end;
