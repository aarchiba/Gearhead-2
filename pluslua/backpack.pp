...
uses ability,action,arenacfe,arenascript,gamebook,ghholder,
     ghmodule,ghprop,ghswag,interact,menugear,rpgdice,skilluse,texutil,
     description,ghweapon,uiconfig,narration,specialsys,ghsupport,
...
begin
	...
	if ( BP_ActiveMenu <> Nil ) and ( BP_Source <> Nil ) then begin
		...
		if N > 0 then begin
			...
			else Part := LocateGearByIndex( BP_Source , N );
...
begin
	...
	if ( BP_ActiveMenu <> Nil ) and ( BP_Source <> Nil ) then begin
		...
		else Part := LocateGearByIndex( BP_Source , CurrentMenuItemValue( BP_ActiveMenu ) );
		if Part <> Nil then begin
			BrowserInterfaceInfo( BP_GB , Part , ZONE_ItemsInfo );
		end;
	end;
...
begin
	...
	if N <> -1 then begin
		...
		EquipItem( GB , LList , PC , LocateGearByIndex( PC , N ) , Item );
...
begin
	...
	if N <> -1 then begin
		...
		InstallItem( GB , TruePC , LocateGearByIndex( PC , N ) , Item );
	end;
end;
...
begin
	...
	while M <> Nil do begin
		if M^.G = GG_Character then begin
			if ( NAttValue( M^.NA , NAG_Location , NAS_Team ) = NAV_LancemateTeam ) and NotAnAnimal( M ) and ( NAttValue( M^.NA , NAG_Narrative , NAS_NID ) <> 0 ) then begin
...
Procedure FHQ_SelectMechaForPilot( GB: GameBoardPtr; NPC: GearPtr );
	{ Select a pilot for the mecha in question. }
	{ Pilots must be characters- they must either belong to the default }
	{ player team or, if they're lancemates, they must have a NID. }
	{ This is to prevent the PC from dominating some sewer rats and }
	{ training them to be pilots. }
var
	RPM: RPGMenuPtr;
...
begin
	...
	if ( NAttValue( NPC^.NA , NAG_Narrative , NAS_NID ) = 0 ) and ( NAttValue( NPC^.NA , NAG_Location , NAS_Team ) <> NAV_DefPlayerTeam ) then begin
...
Procedure InstallSoftware( GB: GameBoardPtr; PC , SW: GearPtr );
	...
	begin
		...
		if N > 0 then begin
			SelectComputer := LocateGearByIndex( PC , N );
		end else begin
			SelectComputer := Nil;
		end;
	end;
...
begin
	Repeat
		...
		{ procedure. }
		if N > 0 then begin
			ThisItemWasSelected( GB , LList , PC , M , LocateGearByIndex( M , N ) );
			{ Restore the display. }
			UpdateBackpack( M );
		end;
	until ( N < 0 ) or ForceQuit;
...
begin
	...
	Repeat
		...
		if N > -1 then begin
			ThisItemWasSelected( GB , LList , PC , Mek , LocateGearByIndex( Mek , N ) );
		end;
	until N = -1;
end;
